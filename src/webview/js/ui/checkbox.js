...
AFRAME.registerComponent('checkbox', {
  ...
  init: function () {
    ...
    // OUTLINE
    this.outline = document.createElement('a-plane-rounded');
    ...
    // INSIDE
    this.inside = document.createElement('a-plane-rounded');
    ...
    //menu event listener
    // this.hitbox.addEventListener('raycaster-intersected', function(e){
  ...
  update: function () {
    ...
    function getTextWidth(el, _widthFactor) {
      ...
      if (v.line) {
        ...
        if (!_widthFactor) { _widthFactor = that.getWidthFactor(el, props.wrapCount); }
      ...
      if (that.data.disabled) {
        let timer = setInterval(function() {
          if (that.checkmark.object3D.children[0]) {
            clearInterval(timer);
            that.updateOpacity(that.checkmark, 0.4);
            that.updateOpacity(that.label, 0.4);
          }
        }, 10)
      } else {
        let timer = setInterval(function() {
          if (that.checkmark.object3D.children[0]) {
            clearInterval(timer);
            that.updateOpacity(that.checkmark, 1);
            that.updateOpacity(that.label, 1);
          }
        }, 10)
      }
    }, 0);
  ...
  getWidthFactor: function(el, wrapCount) {
